...
Table 3. Examples of list wise deletion and pair wise deletion [@alruhaymi2021].
...
2)  Maximum-likelihood imputation [@alruhaymi2021], which is based on a variance-covariance matrix for the variables and uses all of the data points that are currently available to determine what values should be imputed for missing data.
...
In the realm of data imputation, a relatively new method of imputation known as MICE has gained widespread acceptance [@alruhaymi2021]. As a result of this, the MICE technique, in conjunction with multiple imputations, has been investigated and utilized throughout this dissertation. In the following chapter, you will find a more in-depth discussion on the topic of multiple imputations, as well as MICE and its application to the analysis.\`
